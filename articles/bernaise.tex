...
\begin{document}
\title{Bernaise: High-level/high-performance simulation of two-phase electrohydrodynamics in complex domains}
...
\begin{abstract}
  \emph{Bernaise} (\emphy{B}inary
  \emphy{E}lect\emphy{r}ohydrody\emphy{na}m\emphy{i}c Solv\emphy{e}r) is a high-level/high-performance finite element solver of two-phase electrohydrodynamic flow in complex geometries.
...
\begin{multline}
  ...
  % - \tilde\sigma\epsilon \int_{\partial \Omega} \v n \cdot \grad \phi^{k+1} h \, \diff A
...
\begin{multline}
  ...
  % - \int_{\partial\Omega} K_i(\phi^{k+1}) \v n \cdot (\grad c_i^{k+1} + c_i^{k+1} \beta_i' \grad \phi^{k+1} + z_i c_i^{k} \grad V^{k+1} ) \omega_i  \, \diff A = 0
...
\begin{equation}
  ...
  % - \int_{\partial\Omega} \varepsilon(\phi^{k+1}) \v n \cdot \grad V^{k+1}  U \, \diff A
...
%  + \int_{\partial\Omega_\outlet} p_\outlet \v n \cdot \v u \, \diff A \\
  ...
  % - \int_{\partial\Omega} \left[ \tilde\sigma \epsilon \v n \cdot \grad\phi \right] \v u \cdot \grad\phi \, \diff A\\
