...
def method(Lx=4., Ly=4., num_obstacles=12,
           rad=0.5, R=0.35, dx=0.05, seed=121, do_plot=True, **kwargs):
    N = int(np.ceil(Lx/dx))
    ...
    obstacles = [(x_max-0.95*rad, y_max-0.9*rad)]

    for x_c in obstacles:
        # Close to the top right corner of the domain
        if x_c[0] > x_max-rad and x_c[1] > y_max-rad:
            # Is there a lonely patch?
            r2 = (x_max-x_c[0])**2 + (y_max-x_c[1])**2
            if r2 > rad**2:
                # There is a lonely patch!
                theta_top = np.arccos((y_max-x_c[1])/rad)
                theta_right = np.arccos((x_max-x_c[0])/rad)

                drx = rad*np.sin(theta_top)
                dry = rad*np.sin(theta_right)

                x_top = x_c[0] + drx
                y_right = x_c[1] + dry

                # Patch
                curve = rad_points(x_c, rad, dx,
                                   theta_start=np.pi/2-theta_top,
                                   theta_stop=theta_right)[1:-1]
                line_right = line_points((x_max, y_right),
                                         (x_max, y_max), dx)
                line_top = line_points((x_max, y_max),
                                       (x_top, y_max), dx)

                pts = curve + line_right + line_top
